...
class GitHubDownloader
  class << self
    def download_file_to_string(github_token, setup={}, raw=true, link=nil)
      ...
      begin
        options = { accept: "application/vnd.github.v3.#{format}" }
        options[:authorization] = "token #{github_token}" unless github_token.nil?
        downloaded_string = RestClient.get(url, options)
